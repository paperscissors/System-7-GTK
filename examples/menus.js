...
const MenuManager = {
  ...
  hideOthers() {
    ...
    const currentProgram =
      activeWindow.querySelector(".window-title").textContent;
    const windows = Array.from(document.querySelectorAll(".window"));
    ...
    windows.forEach((window) => {
      ...
      if (
        ...
        !window.classList.contains("folder-window")
      ) {
        window.classList.add("hidden");
        window.style.display = "none";
      }
  ...
  getOpenPrograms() {
    ...
    windows.forEach((window) => {
      ...
      if (!programs.has(title)) {
        ...
        programs.get(title).hidden =
          programs.get(title).hidden && window.classList.contains("hidden");
      }
    });
  ...
  getInfo() {
    ...
    const type = activeWindow.classList.contains("folder-window")
      ? "Folder"
      : "Document";
  ...
  getActiveProgram() {
    ...
    const activeWindow = Array.from(document.querySelectorAll(".window")).find(
      (win) => win.classList.contains("active"),
    );
...
MenuManager.switchToProgram = function (programName) {
  ...
  if (programName === "Finder") {
    ...
    windows.forEach((window) => {
      window.style.zIndex = Math.max(
        1,
        parseInt(window.style.zIndex || 1) - newZ,
      );
      window.querySelector(".window-titlebar").style.background =
        "var(--system7-titlebar-inactive)";
    });
